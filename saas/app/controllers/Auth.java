...
public class Auth extends Controller {
    ...
    public static Result auth() {
        ...
        if (loginForm.hasErrors())
            ...
            flash("success","Аутентификация пройдена. Добро пожаловать!");
    ...
    public static Result register() {
        Form<Register> reForm = Form.form(Register.class).bindFromRequest();
        if (reForm.hasErrors())
            return badRequest(register.render(reForm));
        else {
            User user = new User(reForm.get().email,reForm.get().password);
            return redirect(routes.Application.index());
        }
    }
